...
- `src/` — `master_post_stability.m` (main analysis script), `combine_Json.m` (merges the raw `OS_spatial_ev_master*.json` files)
- `data/` — eigenvalue data: `OS_spatial_ev_master*.json` (raw), `combined_data.json`, `combined_data_standardized.json` (the file the analysis script uses), `new.json`

**Paths:** `master_post_stability.m` loads `combined_data_standardized.json` via a relative filename in its configuration section (line 5). Since the data now lives in `data/`, either run MATLAB with `data/` as the working directory, or change the filename in the script to `../data/combined_data_standardized.json`.
