    ...
    begin
        ...
        function pairwise_min_distance(A, B)
            return minimum(abs.(reshape(A, :, 1) .- reshape(B, 1, :)))
        end

        function adapt_zeta_alpha(F, alpha_L_u, alpha_L_l)
            all_eigenvals = vcat(alpha_L_u, alpha_L_l)
            min_distance = pairwise_min_distance(F, all_eigenvals)

            safety = 1.0          # maybe 0.5, 1.0, 2.0
            zeta_max = 1.0
            zeta_min = 1e-10

            return clamp(safety * (min_distance^s_alpha + epsilon), zeta_min, zeta_max)
        end
    ...
    begin
        ...
        filename = "contour_iteration_adaptivezeta_spacingcheck.json"
    end
    ...
    function branch_distance(alpha_L_u, alpha_L_l)
        return pairwise_min_distance(alpha_L_u, alpha_L_l)
    end
    ...
    for k = 1:1500
        ...
        begin
            ...
            dist_u = pairwise_min_distance(F, alpha_L_u)
            dist_l = pairwise_min_distance(F, alpha_L_l)
        ...
        for attempt in 1:5
            ...
            # Build the smoothed trial F contour
            F_trial = [alpha_r[j] + alpha_i_smooth[j] * im for j in 1:N]

            # Check whether the smoothed contour moves too much toward the nearest branch
            dt_ok, toward_amounts, local_allowed_toward, full_moves =
                directional_dt_check(
                    ...
                    global_max_move = 0.04
                )

            if dt_ok && all(move_raw .<= factor .* max.(abs.(alpha_i), 1e-12))
