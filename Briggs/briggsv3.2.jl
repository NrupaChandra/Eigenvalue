    ...
    begin
        ...
        function contour_L_at(omega_i_value)
            return Complex{Float64}[omega_r[j] + omega_i_value * im for j in 1:N]
        end
        ...
        @everywhere function spatial_eigvals_filtered(omega)
            eigvals, _ = couetteflow(nothing, omega, Val(:alpha_collocation), Val(:eigen))
        ...
        @everywhere function track_one_point_from_memory(args)
            j, omega, prev_u, prev_l = args

            eigvals = spatial_eigvals_filtered(omega)

            if isempty(eigvals)
                return j, prev_u, prev_l, Inf, Inf, false
            end

            # Candidate for upper = nearest eigenvalue to previous upper at same index
            du = abs.(eigvals .- prev_u)
            idx_u = argmin(du)
            new_u = eigvals[idx_u]
            jump_u = du[idx_u]

            # Candidate for lower = nearest eigenvalue to previous lower
            dl = abs.(eigvals .- prev_l)

            # prevent exact same eigenvalue being used for both branches if alternatives exist
            if length(eigvals) > 1
                dl[idx_u] = Inf
            end
            ...
            return j, new_u, new_l, jump_u, jump_l, true
        end


        function contour_alpha_L_memory(L_trial, prev_alpha_L_u, prev_alpha_L_l)
            args = [
                (j, L_trial[j], prev_alpha_L_u[j], prev_alpha_L_l[j])
                for j in 1:length(L_trial)
            ]

            results = pmap(track_one_point_from_memory, args)
            sort!(results, by = x -> x[1])

            alpha_u_new = ComplexF64[x[2] for x in results]
            alpha_l_new = ComplexF64[x[3] for x in results]

            max_jump_u = maximum([x[4] for x in results])
            max_jump_l = maximum([x[5] for x in results])
            failed = count(x -> !x[6], results)

            return alpha_u_new, alpha_l_new, max_jump_u, max_jump_l, failed
        end
        #########
        # PLOTS #
        #########
    ...
    begin
        ...
        filename = "contour_iteration_commonzeta_4e-4_conti_identityguard.json"
    end
    ...
    function branch_tracking_valid(alpha_L_u, alpha_L_l;
                               collision_tol = 1e-8,
                               jump_ratio_max = 20.0)

        # Basic finite check
        if any(!isfinite(real(a)) || !isfinite(imag(a)) for a in alpha_L_u)
            return false, "non-finite upper branch"
        end

        if any(!isfinite(real(a)) || !isfinite(imag(a)) for a in alpha_L_l)
            return false, "non-finite lower branch"
        end

        # Upper and lower should not become exactly the same branch
        d_branch = minimum(abs.(alpha_L_u .- alpha_L_l))

        if d_branch < collision_tol
            return false, @sprintf("upper/lower collision: d_branch=%.3e", d_branch)
        end

        # Detect huge local jumps along each tracked branch
        du = abs.(diff(alpha_L_u))
        dl = abs.(diff(alpha_L_l))

        med_u = median(du)
        med_l = median(dl)

        if med_u > 0.0 && maximum(du) > jump_ratio_max * med_u
            return false, @sprintf(
                "upper branch jump too large: max=%.3e median=%.3e",
                maximum(du), med_u
            )
        end

        if med_l > 0.0 && maximum(dl) > jump_ratio_max * med_l
            return false, @sprintf(
                "lower branch jump too large: max=%.3e median=%.3e",
                maximum(dl), med_l
            )
        end
    ...
    function branch_identity_valid(alpha_u_trial, alpha_l_trial,
                                alpha_u_old, alpha_l_old;
                                swap_margin = 0.95)

        same_identity =
            mean(abs.(alpha_u_trial .- alpha_u_old)) +
            mean(abs.(alpha_l_trial .- alpha_l_old))

        swapped_identity =
            mean(abs.(alpha_u_trial .- alpha_l_old)) +
            mean(abs.(alpha_l_trial .- alpha_u_old))

        if swapped_identity < swap_margin * same_identity
            return false, @sprintf(
                "upper/lower identity swap detected: same=%.3e swapped=%.3e",
                same_identity,
                swapped_identity
            )
        end
        ...
        return true, @sprintf(
            "identity ok: same=%.3e swapped=%.3e",
            same_identity,
            swapped_identity
        )
    end
    ...
    for k = 1:1500
        ...
        if omega_i <= omega_lower_bound
            omega_candidate = omega_lower_bound
            L_trial = contour_L_at(omega_candidate)

            alpha_L_u_trial, alpha_L_l_trial = contour_alpha_L_conti(L_trial)

            tracking_ok, tracking_reason =
                branch_tracking_valid(alpha_L_u_trial, alpha_L_l_trial)

            identity_ok, identity_reason =
                branch_identity_valid(alpha_L_u_trial, alpha_L_l_trial,
                                    alpha_L_u, alpha_L_l)

            if !identity_ok
                tracking_ok = false
                tracking_reason = identity_reason
            end

            max_track_jump_u = mean(abs.(alpha_L_u_trial .- alpha_L_u))
            max_track_jump_l = mean(abs.(alpha_L_l_trial .- alpha_L_l))
            n_track_failed = 0
            ...
            if tracking_ok
                ...
                @printf(
                    "[k=%d] STOP: repair step failed branch tracking (%s)\n",
                    k, tracking_reason
                )
                flush(stdout)
                break
            ...
            if !omega_repaired
                for omega_attempt in 1:50
                    ...
                    interior_candidates = omega_i_cache[2:end-1]
                    greater_candidates = filter(x -> isfinite(x) && x > omega_lower_bound, interior_candidates)
                    ...
                    if length(greater_candidates) >= min_valid_omega_candidates
                        ...
                        if omega_jump < 1e-10
                            @printf(
                                "[k=%d] omega rejected: zero/stagnant jump %.3e | reducing dt_omega %.2e -> %.2e\n",
                                k, omega_jump, omega_dt, 0.5 * omega_dt
                            )
                            omega_dt *= 0.5
                            continue
                        end
                        ...
                        if omega_jump <= max_allowed_omega_jump
                            ...
                            if tracking_ok
                                ...
                                @printf(
                                    ...
                                    k, tracking_reason, omega_dt, 0.5 * omega_dt
                                )
                                flush(stdout)
            ...
            # L, alpha_L_u and alpha_L_l were already accepted together
            # inside the guarded omega/repair step above.
            load_on_workers()
        ...
        for attempt in 1:5
            ...
            global omega_F = contour_omega_F(F)
            @printf(
                "[%04d] jump=%9.3e | dUL=%9.3e | dUF=%9.3e | dLF=%9.3e | dt=%9.3e | ζ=%9.3e | trackU=%9.3e | trackL=%9.3e | failed=%d | %s/%s\n",
                k,
                omega_jump,
                d_branch,
                dist_u,
