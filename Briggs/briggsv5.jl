...
begin
    ...
    @everywhere function find_valid_start_index(L, F, normals_F)
        ...
        error("No valid start index found: could not classify upper and lower branches.")
    end
...
for k = 1:1500
    ...
    begin
        ...
        omega_repair_margin = 20 * omega_clearance
        omega_lower_bound = omegaF_max_i + omega_clearance
        omega_repair_target = omegaF_max_i + omega_repair_margin
        ...
        if omega_i <= omega_lower_bound
            global omega_i = omega_lower_target
            global L = contour_L()
            load_on_workers()
        ...
        if omega_status == "blocked"|| omega_status == "repaired"
    ...
    for attempt in 1:8
        ...
        global omega_F = contour_omega_F(F)
        @printf(
            "[%04d] jump=%9.3e | dUL=%9.3e | dUF=%9.3e | dLF=%9.3e | dtF=%9.3e | dtω=%9.3e | facω=%6.3f | ζ=%9.3e | %s/%s\n",
            k,
            omega_jump,
            d_branch,
            dist_u,
