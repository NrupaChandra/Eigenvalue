# briggsv4.1_ribbon.jl -- briggsv4.jl DESCENT kept byte-identical (so the pinch
# forms and dUL closes exactly as in the working Couette run / contour_video13),
# with ONE additive change for the vibrating ribbon:
#   the SAVED per-frame contour is the NEAT ribbon (Hankel) contour that detours
#   around the real forcing frequency omega_f, with its branches recomputed by
#   robust per-point classification purely for the animation.
# The descent engine (matrices, tracking, guards, lambda=4.0, sigma=3e-5,
# delta_t=1e-3, zeta_common=4e-4, straight descending L, pinch_tol, loop length)
# is UNCHANGED. omega_f only positions the drawn detour; it does not steer the
# descent -- the pinch it finds is the flow's intrinsic pinch (Im(omega0)<0 =>
# not absolutely unstable). Run from Briggs/vib_ribbon/:  julia briggsv4.1_ribbon.jl
...
begin
    ...
    function hankel_L(omega_i_level)
        ...
        n_hor_left = 110; n_ver_left = 25; n_arc = 40; n_ver_right = 25; n_hor_right = 110
...
begin
    ...
    filename = "contour_iteration_v4.1_ribbon.json"
end

# ---- ribbon animation frame: Hankel contour + per-point display branches ----
function backfill_nan!(v)
    good = findall(z -> isfinite(real(z)) && isfinite(imag(z)), v)
    isempty(good) && return v
    for j in eachindex(v)
        if !(isfinite(real(v[j])) && isfinite(imag(v[j])))
            v[j] = v[good[argmin(abs.(good .- j))]]
        end
    end
    return v
end
function branches_over_contour(Lc)
    res = pmap(om -> dominant_eigvals(om, F, normals_F), Lc)
    au = ComplexF64[]; al = ComplexF64[]
    for (uu, ll, _, _) in res
        push!(au, uu === nothing ? ComplexF64(NaN, NaN) : ComplexF64(uu))
        push!(al, ll === nothing ? ComplexF64(NaN, NaN) : ComplexF64(ll))
    end
    return backfill_nan!(au), backfill_nan!(al)
end
function make_ribbon_frame(iter)
    Lh = hankel_L(omega_i)
    au, al = branches_over_contour(Lh)
    return Dict(
        "iteration" => iter,
        "omega_f"   => omega_f,
...
begin
    iteration_step = 1
    dict_to_JSON = make_ribbon_frame(iteration_step)
    current_array = Any[]
...
for k = 1:1500
    ...
    for attempt in 1:5
        ...
        flush(stdout)
        load_on_workers()
        local dict_to_JSON = make_ribbon_frame(iteration_step)
