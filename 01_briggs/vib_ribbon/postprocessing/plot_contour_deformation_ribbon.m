...
for k = 1:numIterations
    ...
    if ~isempty(d.alpha_L_u_descent)
        all_alpha = [all_alpha; d.alpha_L_u_descent(:); d.alpha_L_l_descent(:)];
    end
end
...
function data = read_contour_integration_json(filename)
    ...
    for k = 1:numIterations
        ...
        % Descent diagnostics: present from v4.2 onward
        if isfield(entry, 'L_descent')
            data(k).L_descent         = reim_to_complex(entry.L_descent);
            data(k).alpha_L_u_descent = reim_to_complex(entry.alpha_L_u_descent);
            data(k).alpha_L_l_descent = reim_to_complex(entry.alpha_L_l_descent);
        else
            data(k).L_descent         = [];
            data(k).alpha_L_u_descent = [];
            data(k).alpha_L_l_descent = [];
        end
    end
end
...
function make_contour_video(jsonfile, outputfile, omega_xlim, omega_ylim, alpha_xlim, alpha_ylim)
    ...
    for k = 1:numIterations
        ...
        plot_temporal(h1, L, omega, d.omega_f, omega_xlim, omega_ylim, k);
        plot_spatial(h2, F, alphaU, alphaL, d.alpha_L_u_descent, d.alpha_L_l_descent, alpha_xlim, alpha_ylim, k);
...
function plot_temporal(ax, L, omega_F, omega_f, xlimits, ylimits, k)
    ...
    % Forcing-frequency marker on the real axis (no analytical pinch exists
    % for this case, so no exact-pinch marker is drawn)
    plot(omega_f, 0, ...
        'k^', ...
        'MarkerFaceColor', 'k', ...
        'MarkerSize', 6, ...
...
function plot_spatial(ax, F, alphaU, alphaL, alphaU_desc, alphaL_desc, xlimits, ylimits, k)
    ...
    % Descent-tracked branches first (underneath, gray)
    if ~isempty(alphaU_desc)
        vUd = isfinite(real(alphaU_desc)) & isfinite(imag(alphaU_desc));
        vLd = isfinite(real(alphaL_desc)) & isfinite(imag(alphaL_desc));

        plot(real(alphaU_desc(vUd)), imag(alphaU_desc(vUd)), ...
            '-', ...
            'Color', [0.55, 0.55, 0.55], ...
            'LineWidth', 1.0, ...
            'DisplayName', '\alpha_L^u (descent)');

        plot(real(alphaL_desc(vLd)), imag(alphaL_desc(vLd)), ...
            '--', ...
            'Color', [0.55, 0.55, 0.55], ...
            'LineWidth', 1.0, ...
            'DisplayName', '\alpha_L^l (descent)');
    end
