# briggsv5.2_ribbon.jl -- briggsv4.6_ribbon.jl with ONE change: N = 100 -> 300.
#
# Nothing else is touched. No new diagnostics, no new stopping rules, no changes
# to the tracker, the seed, the contour ranges, num_modes, da_max, the guards or
# the iteration count. This is v4.6 run at a finer contour resolution, so that
# the effect of the resolution alone can be seen without anything else moving.
#
# N sets both the F contour and the omega_r grid the ribbon horizontals are
# built on:
#     d(omega_r) on the horizontals = 0.5 / (N-1) / n_sub
#         N = 100 (v4.6)  ->  2.53e-3
#         N = 300 (here)  ->  8.36e-4
#     n_L = 570 -> 954,  F = 100 -> 300 points
# Cost is roughly 1.7x the spatial solves and 3x the temporal solves per frame.
#
# Why this value: in the v4.6 post-mortem, continuing alpha along the left
# horizontal from the seed to omega_r = 0.24 at omega_i = -0.05130 gave
#     48 nodes (v4.6 spacing)   0.65360 + 0.08051i   <- what v4.6 produced
#     96 nodes                  0.40647 + 0.00375i
#    192 nodes                  0.40647 + 0.00375i   <- converged
# so 2.53e-3 was too coarse and ~1.25e-3 was enough. 8.36e-4 sits below that.
#
# Note for whoever runs this: num_modes is left at 150 deliberately, because
# "rest all the same" is the point of this version. Be aware that 150 is past
# the plateau of this discretisation -- max|D4| grows like n^8, and at
# omega = 0.15075 - 0.05422i one eigenvalue reads 0.34692 - 0.01214i for
# num_modes = 50..100 (spread 4e-7) but 0.35221 - 0.01057i at 150, an error of
# 5.5e-3. If this run shows trouble in the alpha ~ 0.3 region, num_modes = 80 is
# a one-line thing to try next, on its own.
#
# ---------------------------------------------------------------------------
# Inherited description (v4.6), unchanged:
#
...
# Run from vib_ribbon/src/:  julia briggsv5.2_ribbon.jl
...
begin
    ...
    filename = "contour_iteration_v5.2_ribbon.json"
end
...
function ribbon_branches(Lc)
    ...
    global last_track_diag = (add_u, nfb_u, add_l, nfb_l)
    return au, al
end
