# briggsv5_ribbon.jl -- v4.6 with the four fixes the v4.6 post-mortem called
...
# Run from vib_ribbon/src/:  julia briggsv5_ribbon.jl
...
begin
    ...
    # interior seed node (the descent's own start_index = N/4), as a contour index
    i_seed_coarse = map_hl[max(1, min(length(map_hl), floor(Int, N / 4)))]
...
begin
    ...
    filename = "contour_iteration_v5_ribbon.json"
end
...
begin
    ...
    @printf("v5 setup: N=%d, n_L=%d, horizontal d(omega_r)=%.3e, DA_MAX=%.3g, num_modes=%d\n",
            N, length(L), (omega_r[2] - omega_r[1]) / n_sub, DA_MAX, num_modes)
