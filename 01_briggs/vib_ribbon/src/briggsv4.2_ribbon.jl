# briggsv4.2_ribbon.jl -- same DESCENT as briggsv4.1_ribbon.jl (byte-identical to
# briggsv4.jl: matrices, tracking, guards, lambda=4.0, sigma=3e-5, delta_t=1e-3,
# straight descending L, pinch_tol, loop length), with the ANIMATION fixed:
#
# v4.1 problem: the saved per-frame ribbon branches were classified INDEPENDENTLY
# at every point of the Hankel contour (dominant_eigvals per point, no continuity),
# so the display branches mode-hopped -- on the vertical risers essentially every
# point jumped to a different eigenvalue (jumps up to ~1.5 in |alpha|), which is
# the noise visible in contour_video4.1ribbon.mp4. The real (tracked) descent
# branches, which DO pinch smoothly, were never saved.
#
# v4.2 changes (animation only, descent untouched):
#   1. display branches are continuity-tracked along the Hankel contour: full
#      spatial spectra are computed in parallel, then each branch is followed
#      serially by nearest-neighbour continuation with a damped linear predictor,
#      seeded from the descent's tracked branches at the left contour end;
#   2. riser resolution increased (25 -> 60 points each) so the continuation has
#      small steps on the vertical segments;
#   3. the descent's own straight contour and tracked branches are saved per frame
#      (L_descent, alpha_L_u_descent, alpha_L_l_descent) for verification;
#   4. the JSON frame array is kept in memory instead of re-reading/re-parsing the
#      whole file every iteration.
# No analytical pinch point exists for this case; the animation carries no exact
# pinch marker (see ../postprocessing/plot_contour_deformation_ribbon.m).
# omega_f only positions the drawn detour; it does not steer the descent.
# Run from vib_ribbon/src/:  julia briggsv4.2_ribbon.jl
...
begin
    ...
    function hankel_L(omega_i_level)
        ...
        # horizontals + vertical risers + semicircle of radius r_arc over omega_f.
        n_hor_left = 110; n_ver_left = 60; n_arc = 40; n_ver_right = 60; n_hor_right = 110
...
begin
    ...
    filename = "contour_iteration_v4.2_ribbon.json"
end

# ---- ribbon animation frame: Hankel contour + continuity-tracked branches ----
...
function track_display_branch(spectra, seed)
    n = length(spectra)
    out = Vector{ComplexF64}(undef, n)
    prev = ComplexF64(seed)
    drift = 0.0 + 0.0im
    for j in 1:n
        pred = prev + 0.5 * drift   # damped predictor: robust at contour corners
        evs = spectra[j]
        best = evs[argmin(abs.(evs .- pred))]
        drift = (j == 1) ? (0.0 + 0.0im) : (best - prev)
        out[j] = best
        prev = best
    end
    return out
...
function make_ribbon_frame(iter)
    ...
    # Seed with the descent's tracked branch values at omega = omega_r_start + im*omega_i,
    # which is exactly the first point of the Hankel contour.
    seed_u = alpha_L_u[1] === nothing ? dominant_eigvals(Lh[1], F, normals_F)[1] : alpha_L_u[1]
    seed_l = alpha_L_l[1] === nothing ? dominant_eigvals(Lh[1], F, normals_F)[2] : alpha_L_l[1]
    au = track_display_branch(spectra, seed_u)
    al = track_display_branch(spectra, seed_l)
    return Dict(
        "iteration" => iter,
        "omega_f"   => omega_f,
        ...
        # descent diagnostics: the straight contour + tracked branches driving the run
        "L_descent"         => complexvec_to_json(L),
        "alpha_L_u_descent" => complexvec_to_json(descent_branch_clean(alpha_L_u)),
        "alpha_L_l_descent" => complexvec_to_json(descent_branch_clean(alpha_L_l)),
    )
end
...
begin
    iteration_step = 1
    ribbon_frames = Any[]
    push!(ribbon_frames, make_ribbon_frame(iteration_step))
    open(filename, "w") do file
        write(file, JSON.json(ribbon_frames))
    end
    iteration_step += 1
end
...
for k = 1:1500
    ...
    global omega_i, L, alpha_L_u, alpha_L_l, alpha_i, F, omega_F, iteration_step, ribbon_frames
