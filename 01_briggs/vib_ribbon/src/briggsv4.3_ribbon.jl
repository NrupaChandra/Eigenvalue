...
# Secant predictor with the predicted displacement clamped to da_max, then a
# nearest-eigenvalue corrector. A node is flagged when the accepted step is too
# large or when the winner is not clearly nearer than the runner-up.
function continue_branch(nodes, spectra, seed; da_max = 0.03, sep_frac = 0.6)
    n = length(nodes)
    ...
    for j in 1:n
        ...
        if have2
            ...
            if abs(dwp) > 1e-14
                ...
                if abs(disp) > da_max              # clamp a runaway extrapolation
        ...
        if isempty(evs) || !isfin(pred)
            ...
            cand = evs[k1]
            vals[j] = cand
            if abs(cand - a_prev) > da_max || d1 > sep_frac * d2
                flag[j] = true
            end
        end
...
function track_path(nodes0, spectra0, seed; max_refine = 4, node_cap = 1200,
    ...
    for _ in 1:max_refine
        ...
        nn = ComplexF64[]; ss = Vector{Vector{ComplexF64}}(); oo = Bool[]
        p = 1
        for j in 1:length(nodes)
            while p <= length(ins_at) && ins_at[p] == j
                push!(nn, ins_w[p]); push!(ss, new_spec[p]); push!(oo, false); p += 1
            end
