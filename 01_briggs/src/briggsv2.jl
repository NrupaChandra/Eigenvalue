    ...
    begin
        ...
        function contour_omega_F(F)
            ...
            for (j, alpha) in enumerate(F)
                try
                    eigval, eigvec = couetteflow_temporal_sing_mode(alpha)
                    push!(omega_F, eigval)
                catch e
                    @printf("\nERROR while computing omega_F\n")
                    @printf("failed index j = %d\n", j)
                    @printf("alpha = %.16e %+.16ei\n", real(alpha), imag(alpha))
                    @printf("error = %s\n\n", string(e))
                    rethrow(e)
                end
            end
    ...
    #################
    begin
        lambda = 3.0       # much stronger downward push of omega_i_cache[j] =
        sigma = 5e-5       # weaker smoothing/diffusion on F
        delta_t = 2e-3     # aggressive initial pseudo-time step
        ...
        filename = "contour_iteration_adaptivezeta_trackingguardv2.json"
    end
    ...
    for k = 1:1500
                ...
                for omega_attempt in 1:50
                    ...
                    if omega_jump < 1e-10
                        ...
                        if omega_jump <= max_allowed_omega_jump
                            ...
                            # ------------------------------------------------------------
                            # Trial branch tracking check before accepting omega step
                            # ------------------------------------------------------------
                            L_trial = contour_L_at(omega_candidate)
        ...
        for attempt in 1:5
            ...
            #println("final zeta_alpha = ", zeta_alpha)
            ...
            global omega_F = contour_omega_F(F)
            @printf(
                "[k=%d] omega_i %.6e -> %.6e | jump=%.2e | valid=%d | dt_omega=%.2e | %s\n",
                k, omega_i_old, omega_i, omega_jump, omega_valid_count, omega_dt_used, omega_status
            )

            @printf(
                "       distances: branch=%.3e | contour=%.3e | upper=%.3e | lower=%.3e\n",
                d_branch, d_contour, dist_u, dist_l
            )
            @printf(
                "       F: raw=%.3e | smooth=%.3e | zeta_alpha=%.3e | attempt=%d | %s\n",
                max_raw_move, max_smooth_move, zeta_alpha, alpha_attempt_used, alpha_status
            )

            @printf(
                "       adaptive: branch_factor=%.3e | local_delta_t=%.3e\n",
                branch_factor, local_delta_t
            )
            flush(stdout)
            load_on_workers()
    ...
    # iteration_step, L, alpha_L_u, alpha_L_l, F, omega_F = load_step("contour_iteration_aggressive.json"; offset=0)
