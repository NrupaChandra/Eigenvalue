...
begin
    ...
    filename = "contour_iteration_v4.5.json"
end
...
# never fire.  3e-3 is just clear of it.
const PINCH_TOL          = 3.0e-3
...
# The history is cleared on every refinement, so indices are comparable
# within a window (omega_r cannot change inside one).
...
for k = 1:2000
    ...
    for attempt in 1:5
        ...
        global omega_F = contour_omega_F(F)
        @printf(
            "[%04d] jump=%9.3e | dUL=%9.3e | dUF=%9.3e | dLF=%9.3e | dt=%9.3e | z=%9.3e | gap=%9.3e (%s,%d) | NL=%3d L%d h=%8.2e | %s/%s\n",
            k, omega_jump, d_branch, dist_u, dist_l, local_delta_t, zeta_common,
            omega_gap_after, omega_bisect_status, omega_bisect_tries,
            length(omega_r), adapt_level, h_local,
            ...
            if adapt_stalled()
                ...
                elseif i_pinch <= 1 || i_pinch >= length(omega_r)
                    ...
                    elseif length(omega_r) + n_add > ADAPT_MAX_POINTS
                        ...
                        @printf(
                            "[k=%d] ADAPT -> level %d | window [%.9f, %.9f] | h %.3e -> %.3e | N_L %d -> %d | |w''|~%.4f | d %.4e -> %.4e (gain %.3f, predicted %.4e)\n",
                            k, adapt_level, w_lo, w_hi, h_local, h_new,
                            n_before, length(omega_r), w2, d_branch, d_after, gain, dpre)
                        flush(stdout)
