...
begin
    ...
    filename = "contour_iteration_v4.6.json"
end
...
for k = 1:2000
    ...
    for attempt in 1:5
        ...
        global omega_F = contour_omega_F(F)
        @printf(
            "[%04d] jump=%9.3e | dUL=%9.3e | dUF=%9.3e | dLF=%9.3e | dt=%9.3e | z=%9.3e | gap=%9.3e (%s,%d) | NL=%3d L%d h=%8.2e gs=%5.2f miss=%8.2e | %s/%s\n",
                ...
                elseif i_pinch <= 1 || i_pinch >= length(omega_r)
                    ...
                    elseif length(omega_r) + n_add > ADAPT_MAX_POINTS
                        ...
                        grid_limited = !ADAPT_SCORE_GATE ||
                                       (isfinite(grid_share) && grid_share >= ADAPT_GRID_SHARE)
                        ...
                        if !grid_limited
                            @printf("[k=%d] adapt: round NOT scored -- horizontal miss explains %.0f %% of d_branch (< %.0f %%), so omega_i is still the limiter | adapt_nogain stays %d\n",
                                    k, 100 * grid_share, 100 * ADAPT_GRID_SHARE, adapt_nogain)
                            flush(stdout)
